...
✅ Your private keys remain on this system and are never transmitted."""
    ...
    def _create_existing_user_view(self, parent):
        ...
        try:
            ...
            # Get user stats from production database
            try:
                with self.app_backend.db.pool.get_connection() as conn:
                    ...
                    cursor = conn.execute("""
                        ...
                        FROM files f 
                        JOIN folders fo ON f.folder_id = fo.folder_unique_id
                    """)
...
def main():
    """Test the dialog with real backend (development only)"""
    import sys
    import os
    ...
    try:
        ...
        dialog = UserInitDialog(root, backend)
