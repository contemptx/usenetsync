...
def main():
    """Test the panel with real backend (development only)"""
    import sys
    import os
    ...
    try:
        ...
        # Get first available folder for testing
        with backend.db.pool.get_connection() as conn:
            cursor = conn.execute("SELECT folder_unique_id FROM folders LIMIT 1")
            row = cursor.fetchone()
            
            if row:
                folder_id = row[0]
                panel = FolderDetailsPanel(root, backend, folder_id)
                panel.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)
            else:
                ttk.Label(root, text="No folders found. Please index a folder first.").pack(pady=20)
        
    except Exception as e:
        ttk.Label(root, text=f"Error initializing backend: {e}").pack(pady=20)
        print(f"Cannot test without proper backend setup: {e}")
