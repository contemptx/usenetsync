#!/usr/bin/env python3
"""
Test ALL functionality of the UsenetSync application
"""
...
def test_functionality():
    ...
    print("=" * 80)
    print("TESTING ALL USENETSYNC FUNCTIONALITY")
    print("=" * 80)
    ...
    # Add folder
    print(f"Testing: add_folder (path: {test_dir})")
    ...
    # Index folder
    if folder_id:
        print(f"Testing: index_folder (folder_id: {folder_id})")
    ...
    # Get folder info
    if folder_id:
        print(f"Testing: folder_info (folder_id: {folder_id})")
    ...
    if folder_id:
        print(f"Testing: segment_folder (folder_id: {folder_id})")
    ...
    # Get share details
    if share_id:
        print(f"Testing: get_share_details (share_id: {share_id})")
    ...
    if folder_id:
        # Set folder access
        print(f"Testing: set_folder_access (folder_id: {folder_id})")
    ...
    if folder_id:
        print(f"Testing: upload_folder (folder_id: {folder_id})")
        result = call_backend('upload_folder', {'folder_id': folder_id})
        results['upload_folder'] = result.get('success', False)
        print(f"  Result: {result}")
        
        print(f"Testing: publish_folder (folder_id: {folder_id})")
    ...
    if share_id:
        print(f"Testing: download_share (share_id: {share_id})")
    ...
    if passed == total:
        ...
        print("\n⚠️ Some functionality is working, but there are issues.")
    else:
        print("\n❌ Many tests failed. The application needs fixes.")
