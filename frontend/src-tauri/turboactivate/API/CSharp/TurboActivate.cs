...
{        /*
    ...
    */
    ...
    public class TurboActivate
    {
...
#if TA_BOTH_DLL
            handle = IntPtr.Size == 8 ? Native64.TA_GetHandle(vGUID) : Native.TA_GetHandle(vGUID);
#else
        ...
        delegate void TrialCallbackType(UInt32 status);
        ...
        event TrialCallbackHandler privTrialChange;
        ...
        TrialCallbackType privTrialCallback;
...
#if TA_BOTH_DLL
                    switch (IntPtr.Size == 8 ? Native64.TA_SetTrialCallback(handle, privTrialCallback) : Native.TA_SetTrialCallback(handle, privTrialCallback))
#else
        ...
        static class Native
        {
            ...
            [DllImport("TurboActivate.dll", CharSet = CharSet.Unicode, CallingConvention = CallingConvention.Cdecl)]
#endif
            public static extern int TA_SetTrialCallback(UInt32 handle, TrialCallbackType callback);
            ...
            [DllImport("TurboActivate.dll", CharSet = CharSet.Unicode, CallingConvention = CallingConvention.Cdecl)]
#endif
            public static extern int TA_Cleanup();
        ...
        */
        ...
        const int TA_E_INSUFFICIENT_BUFFER = 0x0E; // never should get this in our code
        ...
        const int TA_E_BROKEN_WMI = 0x22;
        const int TA_E_INET_TIMEOUT = 0x23;
        const int TA_E_INET_TLS = 0x24;
        ...
        void TrialCallbackFn(UInt32 status)
        {
            TA_TrialStatus lStatus = (TA_TrialStatus)status;

            // call the event from the calling thread's context
            var e = new StatusArgs { Status = lStatus };

            // finish off the operation
            _operation.Post(RaiseTrialCallbackFn, e);
        }

        void RaiseTrialCallbackFn(object args)
        {
            StatusArgs stArgs = (StatusArgs)args;
...
#if TA_BOTH_DLL
            ret = IntPtr.Size == 8 ? Native64.TA_ActivateFromFile(handle, filename) : Native.TA_ActivateFromFile(handle, filename);
#else
...
#if TA_BOTH_DLL
            ret = IntPtr.Size == 8 ? Native64.TA_CheckAndSavePKey(handle, productKey, flags) : Native.TA_CheckAndSavePKey(handle, productKey, flags);
#else
...
#if TA_BOTH_DLL
            ret = IntPtr.Size == 8 ? Native64.TA_Deactivate(handle, (byte)(eraseProductKey ? 1 : 0)) : Native.TA_Deactivate(handle, (byte)(eraseProductKey ? 1 : 0));
#else
...
#if TA_BOTH_DLL
            ret = IntPtr.Size == 8 ? Native64.TA_DeactivationRequestToFile(handle, filename, (byte)(eraseProductKey ? 1 : 0)) : Native.TA_DeactivationRequestToFile(handle, filename, (byte)(eraseProductKey ? 1 : 0));
#else
        ...
        {
#if TA_BOTH_DLL
            int ret = IntPtr.Size == 8 ? Native64.TA_IsActivated(handle) : Native.TA_IsActivated(handle);
#else
        ...
        {
#if TA_BOTH_DLL
            int ret = IntPtr.Size == 8 ? Native64.TA_IsDateValid(handle, date_time, flags) : Native.TA_IsDateValid(handle, date_time, flags);
#else
        ...
        {
#if TA_BOTH_DLL
            int ret = IntPtr.Size == 8 ? Native64.TA_IsGenuine(handle) : Native.TA_IsGenuine(handle);
#else
...
#if TA_BOTH_DLL
            ret = IntPtr.Size == 8 ? Native64.TA_IsGenuineEx(handle, ref opts) : Native.TA_IsGenuineEx(handle, ref opts);
#else
...
#if TA_BOTH_DLL
            int ret = IntPtr.Size == 8 ? Native64.TA_GenuineDays(handle, daysBetweenChecks, graceDaysOnInetErr, ref daysRemain, ref inGrace) : Native.TA_GenuineDays(handle, daysBetweenChecks, graceDaysOnInetErr, ref daysRemain, ref inGrace);
#else
...
#if TA_BOTH_DLL
            int ret = IntPtr.Size == 8 ? Native64.TA_TrialDaysRemaining(handle, useTrialFlags, ref daysRemain) : Native.TA_TrialDaysRemaining(handle, useTrialFlags, ref daysRemain);
#else
        ...
        {
#if TA_BOTH_DLL
            int ret = IntPtr.Size == 8 ? Native64.TA_UseTrial(handle, flags, extraData) : Native.TA_UseTrial(handle, flags, extraData);
#else
        ...
        {
#if TA_BOTH_DLL
            int ret = IntPtr.Size == 8 ? Native64.TA_UseTrialVerifiedRequest(handle, filename, extraData) : Native.TA_UseTrialVerifiedRequest(handle, filename, extraData);
#else
        ...
        {
#if TA_BOTH_DLL
            int ret = IntPtr.Size == 8 ? Native64.TA_UseTrialVerifiedFromFile(handle, filename, flags) : Native.TA_UseTrialVerifiedFromFile(handle, filename, flags);
#else
        ...
        {
#if TA_BOTH_DLL
            int ret = IntPtr.Size == 8 ? Native64.TA_ExtendTrial(handle, useTrialFlags, trialExtension) : Native.TA_ExtendTrial(handle, useTrialFlags, trialExtension);
#else
        ...
        {
#if TA_BOTH_DLL
            int ret = IntPtr.Size == 8 ? Native64.TA_SetCustomActDataPath(handle, directory) : Native.TA_SetCustomActDataPath(handle, directory);
#else
        ...
        /// <summary>You should call this before your application exits. This frees up any
        /// allocated memory for all open handles.</summary>
        public static void Cleanup()
        {
#if TA_BOTH_DLL
            switch (IntPtr.Size == 8 ? Native64.TA_Cleanup() : Native.TA_Cleanup())
#else
            switch (Native.TA_Cleanup())
#endif
            {
                case TA_OK: // successful
                    return;
                default:
                    throw new TurboActivateException("Cleanup failed.");
            }
        }
...
#if TA_BOTH_DLL
            int ret = IntPtr.Size == 8 ? Native64.TA_GetVersion(out major, out minor, out build, out rev) : Native.TA_GetVersion(out major, out minor, out build, out rev);
#else
            int ret = Native.TA_GetVersion(out major, out minor, out build, out rev);
#endif
    ...
    public class InternetTimeoutException : InternetException
    {
        public InternetTimeoutException()
            : base("The connection to the server timed out because a long period of time elapsed since the last data was sent or received.")
        { }
    }
    ...
    public class InternetTLSException : InternetException
    {
        ...
        public InternetTLSException()
            : base("The secure connection to the activation servers failed due to a TLS or certificate error. More information here: https://wyday.com/limelm/help/faq/#internet-error")
        {
        }
    }
    ...
    public class DateTimeException : TurboActivateException
    {
        public DateTimeException()
            : base("The activation has expired or the system time has been tampered with. Ensure your time, timezone, and date settings are correct. After fixing them restart your computer.")
        {
        }
    }
    ...
    public class BrokenWMIException : TurboActivateException
    {
        public BrokenWMIException()
            : base("The WMI repository on the computer is broken. To fix the WMI repository see the instructions here: https://wyday.com/limelm/help/faq/#fix-broken-wmi")
        {
        }
    }
